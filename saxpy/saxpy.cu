...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    float* host_y;
    float* host_result;
    cudaHostAlloc((void**)&host_x, N * sizeof(float), cudaHostAllocDefault);
    cudaHostAlloc((void**)&host_y, N * sizeof(float), cudaHostAllocDefault);
    cudaHostAlloc((void**)&host_result, N * sizeof(float), cudaHostAllocDefault);

    memcpy(host_x, xarray, N * sizeof(float));
    memcpy(host_y, yarray, N * sizeof(float));
    ...
    double cudaDuration = cudaEndTime - cudaStartTime;
    double overallDuration = endTime - startTime - cudaDuration;
    printf("Effective BW by CUDA saxpy: %.3f ms\t\t[%.3f GB/s]\n", 1000.f * overallDuration, GBPerSec(totalBytes, overallDuration));
    printf("Effective CUDA compyte time: %.3f ms\n", 1000.f * cudaDuration);
    //
